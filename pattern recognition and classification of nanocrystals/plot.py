...
csfont = {'fontname':'Times New Roman','weight' : 'bold','size'   : 52}
...
for x in range(0,31): 
  ...
  Thermo=np.genfromtxt('thermo.{0}.lammps'.format(x))[:206000]
  PE_ave=np.mean(Thermo[200000:206000,3])
  T_ave =np.mean(Thermo[200000:206000,1])
  print (PE_ave, T_ave);allPE.append(PE_ave);allT.append(T_ave)
  Thermo[:20000,0]=Thermo[:20000,0]/1000000
  #plt.plot(Thermo[:20000,0],Thermo[:20000,3],alpha=0.3,label='real')
...
plt.xlabel('Temperature (K)',**csfont)
#plt.legend(fontsize=20)
#plt.title('PE(eV)'.format(x),**csfont)
plt.savefig('PEvsT.jpg', dpi=300) 
plt.show()
#plt.savefig('graph/Temp{0}.jpg'.forma
